...
PSDBV1_OUT := $(PSDB_OUT)/v1

PROTO_SRC := proto-src
PSDB_SRC := $(PROTO_SRC)/psdb

PSDBV1_SRC := $(PSDB_SRC)/v1
...
$(BIN)/staticcheck: Makefile | $(BIN)
	$(TOOL_INSTALL) honnef.co/go/tools/cmd/staticcheck@v0.4.2
...
proto: \
	$(PSDBV1_OUT)/database.pb.go

$(PSDBV1_OUT)/database.pb.go: $(PROTO_TOOLS) $(PSDBV1_SRC)/database.proto | $(PROTO_OUT)
	$(BIN)/buf generate --path $(PSDBV1_SRC)/database.proto
